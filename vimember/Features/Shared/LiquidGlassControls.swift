...
extension View {
    @ViewBuilder
    func vimemberInteractiveGlass<S: InsettableShape>(in shape: S) -> some View {
        if #available(iOS 26.0, *) {
    ...
    @ViewBuilder
    func vimemberRegularGlass<S: InsettableShape>(in shape: S) -> some View {
        if #available(iOS 26.0, *) {
    ...
    var body: some View {
        ...
        ZStack {
            ZStack(alignment: .topLeading) {
                shape
                    .fill(Color.clear)
                    .vimemberInteractiveGlass(in: shape)
